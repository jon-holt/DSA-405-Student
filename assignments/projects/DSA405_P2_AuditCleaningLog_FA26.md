...
| **Also this window** | **Bench Check 1**, Weeks 5–7. Sign up if not already scheduled. |
| **Time** | 4–5 hours spread across three weeks, not one sitting |
...
verification suite belong to P3. Do not attempt both.
...
**Every row has a number.** "Some rows" is not a number. An uncounted change is an
unknown change.
...
and the first one graded.

**A reader can reverse any single decision** using only the log and the raw file.

Aim for **10 to 20 rows**. Fewer than 8 usually means changes went unlogged.
...
A mismatch means something happened that was not logged; find it. This check has caught
a real error for roughly half of every class that has done it.
...
This section assesses Learning Outcome 6. Presenting any analysis begins with the
...
Raw data is evidence. Overwriting it turns the cleaning log from a verifiable record
into an unverifiable account.
...
single decision. This scores 1 on a ×3 criterion and is the most common way to lose a
grade on P2. Log while working; reconstructing the log afterward misses decisions.

**The log that lists operations.** Twenty rows, every one saying what a line of code
did, none saying why. That is a changelog, not a cleaning log, and it caps the score at
Developing.

## If the data turns out to be clean

Some well-curated sources leave little to fix. That is not a problem and not a reason
to switch datasets in Week 5.
...
If unsure whether this applies, ask the instructor.

...
P2 and P3 grade the dictionary, cleaning log, join, and verification. P4 does not grade those again. P4 assesses the project as a whole: what was found, whether it forms one connected argument, who the data leaves out, and whether a stranger can reproduce it. One P4 row checks that the earlier documentation was *updated* to match the final state. A strong P2 does not raise a weak P4.
...
Websites restructure, APIs are shut down, and terms of use change. If this happens to your project, it will most likely happen in October, while you are building P3.

If a source documented in P1 becomes unavailable through no fault of the student, **notify the instructor within 48 hours** with evidence: the old plan, the new response, and what changed. Documented adaptation is scored as a strength. A pipeline that stopped working weeks earlier and is submitted in November without a note is not.
...
Ungraded, about ten minutes. It helps you find missing pieces before I grade the work.
...
Each milestone builds on the previous one; errors left uncorrected in P2 carry forward into P3.
...
| 2 | **Sources & access evidence** | ×2 | Two or more sources named precisely (publisher, URL, coverage, time span, approximate size), at least one collected from the web. Shows evidence each is reachable: a status code, a row count, a screenshot. Names the part of the plan most likely to fail. | Two sources named specifically, at least one collected from the web, with a plausible stated access path for each. | Sources named vaguely, or the web requirement is unmet, or access is asserted without evidence. | Fewer than two sources, or no access plan that could work. |
...
class works on the Lab. Office hours are available if no slot fits.
...
| 2 | **The ugly problem** | ×2 | Picks a hard problem, shows the evidence of it, and explains the fix well enough that the audience could do it themselves. | Describes a real problem and how it was solved. | Names a problem without showing it, or picks a trivial one. | No problem discussed. |
...
| 4 | **Limitations** | ×1 | Names a specific limitation or exclusion and states what the conclusion cannot claim because of it. | Names a real limitation. | Generic caveat. | None offered. |
| 5 | **Comprehensibility & time** | ×1 | Finishes within three minutes. Visuals legible from the back of the room. The audience can restate the main point afterward. | Close to time, legible visuals, and the audience understands the main point. | Runs long, or visuals unreadable, or the audience cannot tell what the main point is. | Substantially over time, or the talk cannot be followed. |
...
**On Criterion 2:** spend about a third of the three minutes on the ugly problem. A specific account ("finding 4,000 duplicate inspection records created by an inner join") is more useful to the audience, and later in interviews, than a summary of what was built.
